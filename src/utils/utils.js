...
const displayMenu = (originalMenu, currentLevel, clickedItem) => {
  ...
  originalMenu.forEach(treatment => {
    const levels = [treatment.level1, treatment.level2, treatment.level3]
    if (levels[currentLevel] !== ""){
      var index = displayMenu.findIndex(x => x.name === levels[currentLevel])
      if (index === -1){
        if (!clickedItem || (clickedItem && clickedItem["name"] === levels[currentLevel-1])){
          if (levels[currentLevel + 1] !== "" && currentLevel !== 2){
            //Agregar padre
            displayMenu.push({
              price: treatment.price,
              uid: treatment.uid,
              complete_name: levels[0] + " " + levels[1] + " " + levels [2],
